...
def main():
    ...
    # Map job types to GitLab job names (can be single string or list)
    job_map = {
        'ci-tron-job': '.ci-tron-job-v1',
        'ci-tron-b2c-job': '.ci-tron-b2c-job-v1',
        'ci-tron-b2c-diskless-job': ['.ci-tron-b2c-diskless-v1', '.ci-tron-b2c-job-v1']
    }

    target_jobs = job_map.get(job_type)
    if target_jobs is None:
        print(f"ERROR: Unknown JOB_TYPE '{job_type}'. Valid types: {', '.join(job_map.keys())}", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"DEBUG: Target jobs: {target_jobs}", file=sys.stderr)

    # Resolve all variables from all target jobs
    variables = {}
    # Process in reverse order so first job takes precedence
    for target_job in reversed(target_jobs):
        print(f"DEBUG: Processing job: {target_job}", file=sys.stderr)
        print(f"DEBUG: Job extends: {jobs.get(target_job, {}).get('extends', [])}", file=sys.stderr)
        job_vars = resolve_extends(jobs, target_job)
        # Merge variables, later jobs override earlier ones
